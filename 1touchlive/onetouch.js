...
(function() {
	...
	document.addEventListener("appMobi.device.ready", function() {
		...
		if (AppMobi.isfacebook) {
			...
			AppMobi.purchasing = {
				purchaseProduct: function(product_id, quantity, user_id) {
					jsonP("https://cservices.appmobi.com/1touch/OneTouchServices.aspx?cmd=GetGoogleJWT&product_id=" + encodeURIComponent(product_id) + "&quantity=" + encodeURIComponent(quantity) + "&user_id=" + encodeURIComponent(user_id) + "&app=" + encodeURIComponent(AppMobi.app) + "&device=" + encodeURIComponent(AppMobi.device.uuid), function(obj) {
						var data = "product_id:" + product_id + ",quantity:" + quantity + ",user_id:" + user_id + ",app:" + AppMobi.app + ",device:" + AppMobi.device.uuid;
...
var OneTouch = (function() {
	...
	var OneTouch = {
		buy: function(prod_id, quant, cb, failcb, use1Touch) {
			...
			if (!userID || userID == "" || (use1Touch && AppMobi.cache.getCookie("OneTouch_TempID") == "true")) {
				if (use1Touch) {
					...
					var that = this;
					document.addEventListener("appMobi.device.remote.close", OneTouchCloseEvt = function() {}, false);
				} else {
		...
		restore: function(callback) {
			document.addEventListener("appMobi.purchasing.restore", function(){
				callback(arguments[0]);
			}, false);
			AppMobi.purchasing.restorePurchases();
		},
		login: function() {
			if (!available) return AppMobi.notification.alert("1Touch is not available");
			if (!this.appName) return AppMobi.notification.alert("App name is not configured");
